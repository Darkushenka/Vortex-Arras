...
// Define player keys
var keys = [
    'k', 'l', 'TOKEN_KYaPZYAL2vh1yvSYYeZyH6US5bTbOGl_TOKEN',
    // Focus Group
    ...
    let out = entities
      .map(e => {
        if (e.health.amount > 0 && !e.invuln) {
          if (e.master.master.team !== this.body.master.master.team && e.master.master.team !== -101) {
            if (
              e.type === "tank" ||
              e.type === "crasher" ||
              ...
              if (Math.abs(e.x - m.x) < range && Math.abs(e.y - m.y) < range) {
                if (
                  ...
                  (Math.abs(e.x - mm.x) < range && Math.abs(e.y - mm.y) < range)
                )
                  return e;
              }
            }
  ...
  think(input) {
    ...
    for (let i = 0; i < this.body.guns.length; i++) {
      if (this.body.guns[i].canShoot && !this.body.aiSettings.skynet) {
        ...
        tracking = v.speed;
        let calculatedRange = v.speed * v.range;
...
class Gun {
    constructor(body, info) {
        ...
        this.color = (info.PROPERTIES != null && info.PROPERTIES.GUN_COLOR != null) ?
            info.PROPERTIES.GUN_COLOR : null;
    ...
    fire(gx, gy, sk) {
        ...
        if (!isFinite(s.x) || !isFinite(s.y)) { s.x = 0; s.y = 0; }
...
class Entity {
    ...
    define(set) {
        ...
        if (set.PARENT != null) {
            for (let i = 0; i < set.PARENT.length; i++) {
                let parentDef = set.PARENT[i];
                if (parentDef && typeof parentDef === 'object') this.define(parentDef);
            }
        }
        if (set.index != null) {
        ...
        if (set.EXPLOSION_GUNS != null) { 
            let newGuns = [];
            set.EXPLOSION_GUNS.forEach((gundef) => {
                newGuns.push(new Gun(this, gundef));
            });
            this.explosionGuns = newGuns;
        }
    ...
    move() {
        if (this.control.goal == null) {
            ...
            if (t != null && (t.x !== 0 || t.y !== 0) &&
                (this.motionType === 'motor' || this.motionType === 'chase' || this.motionType === 'swarm') &&
                !(this.aiSettings && this.aiSettings.noChase)) {
                this.control.goal = { x: this.x + t.x, y: this.y + t.y };
            } else {
                this.control.goal = { x: this.x, y: this.y };
            }
        }
        let g = {
        ...
        if (this.isDead()) {
            if (this.isDominator) {
                ...
                this.collisionArray.forEach(instance => {
                    ...
                    if (instance.master) killers.push(instance.master);
                });
                let killer = killers.find(k => k.type === 'tank' && k.name !== 'Dominator') || killers[0];
                if (killer) {
    ...
    destroy() {
        ...
        if (this.explosionGuns != null) {
            this.explosionGuns.forEach(gun => {
                gun.fire(0, 0, this.skill);
            });
        }
    ...
    exportDefintionsToClient = (() => { 
        ...
        function getDimensions(entities) {
            ...
            */
            ...
            while (checkingFunction() && maxIterations-- > 0) { // 7) Repeat until we enclose everything
                centerOfCircle = {
        ...
        if (writeData == null) {
            ...
            try {
                fs.writeFileSync(cachePath, JSON.stringify({ key: cacheKey, data: writeData }));
    ...
    return {
        ...
        connect: (() => {
            ...
            function incoming(message, socket) {
                ...
                case 'h': { // chat message
                    if (m.length !== 1) { socket.kick('Ill-sized chat packet.'); return 1; }
                    if (!socket.status.hasSpawned) { return 1; }
                    let text = m[0];
                    if (typeof text !== 'string') { return 1; }
                    text = text.replace(c.BANNED_CHARACTERS_REGEX, '').trim();
                    if (text.length === 0) { return 1; }
                    if (text.length > 84) { text = text.slice(0, 84); }
                    let now = util.time();
                    ...
                    sockets.broadcastChat(name, text);
                } break;
                ...
                case '0': { // testbed cheat
                    ...
                    // cheatingbois
                    if (player.body != null) { if (socket.key === 'TOKEN_KYaPZYAL2vh1yvSYYeZyH6US5bTbOGl_TOKEN') {
                        player.body.define(Class.testbed);
                    } }
                } break;
                case 'T': { // teleport
                    if (m.length !== 2) { socket.kick('Ill-sized teleport request.'); return 1; }
                    // Get data
                    let x = m[0], y = m[1];
                    // Verify data
                    if (typeof x !== 'number' || typeof y !== 'number') { socket.kick('Bad teleport request.'); return 1; }
                    // Teleport
                    if (player.body != null) {
                ...
                case 'P': { // basic tank cheat
                    if (m.length !== 0) { socket.kick('Ill-sized basic tank request.'); return 1; }
                    if (player.body != null) {
                        if (socket.key === 'TOKEN_KYaPZYAL2vh1yvSYYeZyH6US5bTbOGl_TOKEN') {
                ...
                case 'B': { // spawn bot
                    if (m.length !== 0) { socket.kick('Ill-sized bot spawn request.'); return 1; }
                    if (player.body != null) {
                        if (socket.key === 'TOKEN_KYaPZYAL2vh1yvSYYeZyH6US5bTbOGl_TOKEN') {
                            let spot = {
                            ...
                            let bot = new Entity(spot);
                                ...
                                let stats = ['atk','hlt','spd','str','pen','dam','rld','mob','rgn','shi'];
                                while (bot.skill.points > 0) {
                                    let s = ran.choose(stats);
                                    bot.skill.upgrade(s);
                                }
                ...
                case 'V': { // adjust fov
                    if (m.length !== 1) { socket.kick('Ill-sized FOV request.'); return 1; }
                    let dir = m[0];
                    if (typeof dir !== 'number' || (dir !== 1 && dir !== -1)) { socket.kick('Bad FOV request.'); return 1; }
                    if (player.body != null) {
                        if (socket.key === 'TOKEN_KYaPZYAL2vh1yvSYYeZyH6US5bTbOGl_TOKEN') {
                ...
                case 'Y': { // adjust size
                    if (m.length !== 1) { socket.kick('Ill-sized size request.'); return 1; }
                    let dir = m[0];
                    if (typeof dir !== 'number' || (dir !== 1 && dir !== -1)) { socket.kick('Bad size request.'); return 1; }
                    if (player.body != null) {
                        if (socket.key === 'TOKEN_KYaPZYAL2vh1yvSYYeZyH6US5bTbOGl_TOKEN') {
                            if (player.sizeScale == null) player.sizeScale = 1;
                            if (player.baseSize == null) player.baseSize = player.body.SIZE;
                            player.sizeScale *= (dir === 1) ? 1.1 : 1/1.1;
                ...
                case 'I': {
                    if (m.length !== 0) { socket.kick('Ill-sized immortality request.'); return 1; }
                    if (player.body != null) {
                        if (socket.key === 'TOKEN_KYaPZYAL2vh1yvSYYeZyH6US5bTbOGl_TOKEN') {
...
var maintainloop = (() => {
    ...
    (() => {
        let type = ran.choose([Class.destroydom, Class.gunnerdom, Class.trapdom]);
    ...
    let spawnCrasher = census => {
        if (ran.chance(1 -  0.5 * census.crasher / room.maxFood / room.nestFoodAmount)) {
            ...
            let type = (ran.dice(80)) ? ran.choose([Class.sentryGun, Class.sentrySwarm, Class.sentryTrap, Class.layer, , Class.squaredecimatorlite, Class.eggdecimatorlite, Class.minionkiosksentry, Class.rotorkiosksentry]) : Class.crasher;
            let o = new Entity(spot);
                o.define(type);
                o.team = -100;
        }
...
var websockets = new WebSocket.Server({ server: server });
...
setInterval(() => {
    try {
        ...
        util.log(`[INFO] Periodic food clear executed. Removed: ${removed}`);
    } catch (err) {
        util.error('Food clear task error: ' + err?.message);
    }
}, 180000);
