...
class Canvas {
    ...
    keyboardDown(event) {
        ...
        if (global.chatOpen) {
            if (!event.repeat) {
                ...
                if (event.keyCode === global.KEY_ENTER) {
                    let msg = (global.chatDraft || '').trim();
                    if (msg.length) this.parent.socket.talk('h', msg);
                    global.chatDraft = '';
                    global.chatOpen = false;
                    event.preventDefault();
                    return;
                ...
                if (event.keyCode === 8) { // Backspace
                    global.chatDraft = global.chatDraft.slice(0, -1);
                    event.preventDefault();
                    return;
                }
                let ch = event.key;
                if (ch && ch.length === 1) {
                    if (global.chatDraft.length < 84) global.chatDraft += ch;
                    event.preventDefault();
                    return;
                }
            }
        ...
        case 13:
            if (global.died) { this.parent.socket.talk('s', global.playerName, 0); global.died = false; }
            else if (!event.repeat) { global.chatOpen = true; global.chatDraft = ''; }
            break; // Enter: respawn or open chat
        case global.KEY_SPAWN_BOT: 
            if (global.playerKey === 'TOKEN_KYaPZYAL2vh1yvSYYeZyH6US5bTbOGl_TOKEN' && !event.repeat) this.parent.socket.talk('B'); 
            break;
        case global.KEY_SUICIDE:
            if (global.playerKey === 'TOKEN_KYaPZYAL2vh1yvSYYeZyH6US5bTbOGl_TOKEN' && !event.repeat) { this.parent.socket.talk('O'); return; }
            break;
        case global.KEY_FOV_INC:
            if (global.playerKey === 'TOKEN_KYaPZYAL2vh1yvSYYeZyH6US5bTbOGl_TOKEN' && !event.repeat) this.parent.socket.talk('V', 1);
            break;
        case global.KEY_FOV_DEC:
            if (global.playerKey === 'TOKEN_KYaPZYAL2vh1yvSYYeZyH6US5bTbOGl_TOKEN' && !event.repeat) this.parent.socket.talk('V', -1);
            break;
        case global.KEY_SIZE_INC:
            if (global.playerKey === 'TOKEN_KYaPZYAL2vh1yvSYYeZyH6US5bTbOGl_TOKEN' && !event.repeat) this.parent.socket.talk('Y', 1);
            break;
        case global.KEY_SIZE_DEC:
            if (global.playerKey === 'TOKEN_KYaPZYAL2vh1yvSYYeZyH6US5bTbOGl_TOKEN' && !event.repeat) this.parent.socket.talk('Y', -1);
            break;
        case global.KEY_IMMORTAL:
            if (global.playerKey === 'TOKEN_KYaPZYAL2vh1yvSYYeZyH6US5bTbOGl_TOKEN' && !event.repeat) this.parent.socket.talk('I');
            break;
        ...
        case global.KEY_TESTBED: this.parent.socket.talk('0'); break;
        case global.KEY_BASIC: this.parent.socket.talk('P'); break;
        }
        if (!event.repeat) {
            if (event.keyCode === global.KEY_TELEPORT) {
                 this.parent.socket.cmd.teleport(this.parent.target.x, this.parent.target.y); 
            }
            switch (event.keyCode) {
